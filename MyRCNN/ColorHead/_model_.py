...
class SharedConv2d(Module):
    def __init__(self, kernel_size: int=1, stride:int = 0, bias=False,device: device = device("cpu")):
        super().__init__()
        self.kernel_size = kernel_size

        # Single shared kernel
        self.weight = Parameter(
            tensor([0.5], device=device).repeat(kernel_size*kernel_size).reshape(1, 1, kernel_size, kernel_size)
        )
    ...
    def forward(self, x):
        ...
        weight = self.weight.expand(C, 1, self.kernel_size, self.kernel_size)
...
class ColorHead(Module):
    def __init__(self, in_channels: int, half_out_channels: int, device: device = device("cpu")) -> None:
        ...
        self.downgrade = Sequential(
            # Conv2d(in_channels=half_out_channels, out_channels=half_out_channels, kernel_size=3, stride=3, padding=1, bias=False, groups=half_out_channels, device=device)
            SharedConv2d(kernel_size=3, stride=3, bias=False, device=device)
        )
